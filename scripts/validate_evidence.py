...
Editing the canonical skill file and forgetting the mirror re-copy is the #1 CI
failure mode carried over from Skills-for-Langchain; check 2 catches it.
"""
...
PLUGIN_NAME = "skills-for-fastmcp"
SKILL_DIR = "fastmcp"
MARKETPLACE_SOURCE = "./plugins/skills-for-fastmcp"
...
def main() -> None:
    ...
    require(plugin["name"] == PLUGIN_NAME, "Unexpected plugin name", errors)
    require(marketplace["name"] == PLUGIN_NAME, "Unexpected marketplace name", errors)
    require(marketplace["plugins"][0]["name"] == plugin["name"],
            "Marketplace and plugin names must match", errors)
    require(marketplace["plugins"][0]["source"] == MARKETPLACE_SOURCE,
    ...
    canonical = ROOT / ".claude" / "skills" / SKILL_DIR
    packaged = plugin_root / "skills" / SKILL_DIR
